...
{
    public class SystemForITRepository : ISystemForITRepository
    {
        #region 其他
        public SystemForITRepository()
        {
            this.commonForIT = new CommonForIT();
            this._UserInfo = new CommonForIT.User();
        }

        protected CommonForIT commonForIT
        {
            get;
            private set;
        }

        protected CommonForIT.User _UserInfo
        {
            get;
            private set;
        }
        ...
        public List<AccountViewModel> getAccount(string account, string admin)
        {
            ...
            {
                var _admin = db.IFRS9_User.AsNoTracking()
                               .Where(x => x.User_Account.Contains(account), account.IsNullOrWhiteSpace() == false);
            ...
            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                try
                {
                    ...
                    if (actionType == "Add")
                    {
                        ...
                        dataEdit.Create_Date = DateTime.Now.Date;
                        dataEdit.Create_Time = DateTime.Now.TimeOfDay;
                    }
                    else if (actionType == "Modify")
                    {
                        dataEdit = db.IFRS9_Menu_Main
                                     ...
                                     .FirstOrDefault();
                        dataEdit.LastUpdate_Date = DateTime.Now.Date;
                        dataEdit.LastUpdate_Time = DateTime.Now.TimeOfDay;
                    }
        ...
        public List<Tuple<string, string>> getAssessmentAddUser(string productCode, string tableId, SetAssessmentType type)
        {
            ...
            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                ...
                var _debts = commonForIT.getDebt(_debt); //債券類別 A,M,B
                _debts.Add("A");
        ...
        public List<CheckBoxListInfo> getMenu(string userAccount)
        {
            ...
            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                var _debt = commonForIT.getDebt(commonForIT.getUserDebt(userAccount));
                _debt.Add("A");
        ...
        #region
        public List<TaskScheduleViewModel> getTaskSchedule(DateTime? dt,bool addFlag = true)
        {
            List<string> fileNames = new List<string>() {
                ...
                Table_Type.IFRS9Log.ToString()
            };

            List<TaskScheduleViewModel> data = new List<TaskScheduleViewModel>();
            try
            {
                if (addFlag)
                {
                    data.Add(new TaskScheduleViewModel()
                    {
                        Trigger_Task = "StartKRiskService",
                        ReportDate = "",
                        Version = "",
                        TransferType = "Other",
                        Create_Date = "",
                        ...
                        End_Date = "",
                        End_Time = "",
                        File_Name = "StartKRiskService (啟動KRisk Service)"
                    });
                    ...
                    data.Add(new TaskScheduleViewModel()
                    {
                        ...
                        End_Date = "",
                        End_Time = "",
                        File_Name = "StopKRiskService (停止KRisk Service)"
                    });
                    ...
                    data.Add(new TaskScheduleViewModel()
                    {
                        ...
                        End_Date = "",
                        End_Time = "",
                        File_Name = "StartTomcatService (啟動Tomcat Service)"
                    });
                    ...
                    data.Add(new TaskScheduleViewModel()
                    {
                        ...
                        End_Date = "",
                        End_Time = "",
                        File_Name = "StopTomcatService (停止Tomcat Service)"
                    });
                }
                using (IFRS9DBEntities db = new IFRS9DBEntities())
                {
        ...
        public MSGReturnModel restartTaskSchedule(string triggerTask, string reportDate)
        {
            MSGReturnModel result = new MSGReturnModel();
            ...
            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                try
                {
                    string configTxtName = "";

                    switch (triggerTask)
                    {
                        case "StartKRiskService":
                            configTxtName = ConfigurationManager.AppSettings["StartKRiskService"];
                            Process.Start(configTxtName);
                            break;
                        case "StopKRiskService":
                            configTxtName = ConfigurationManager.AppSettings["StopKRiskService"];
                            Process.Start(configTxtName);
                            break;
                        case "StartTomcatService":
                            configTxtName = ConfigurationManager.AppSettings["StartTomcatService"];
                            Process.Start(configTxtName);
                            break;
                        case "StopTomcatService":
                            configTxtName = ConfigurationManager.AppSettings["StopTomcatService"];
                            Process.Start(configTxtName);
                            break;
                        default:
                            //configTxtName = ConfigurationManager.AppSettings["TaskSchedule"];
                            configTxtName = Properties.Settings.Default["TaskScheduleLocation"]?.ToString();
                            Process.Start(configTxtName, $"{triggerTask} {reportDate}");
                            break;
                    }
